...
export default function StudentChatbot({ currentLesson }: { currentLesson?: { subject?: string; number?: number } }) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const { data: result } = await apiClient.post<any>('/api/chat', {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 bg-white rounded-2xl shadow-2xl border border-gray-200 transition-all duration-300 ${isMinimized ? "w-80 h-16" : "w-96 h-[600px]"
        }`}
    >
      { }
        ...
        <>
          ...
          <div className="p-3 bg-gray-50 border-b flex gap-2">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all ${chatMode === "quick"
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all ${chatMode === "deepdive"
                ? "bg-violet-600 text-white shadow-md"
                : "bg-white text-gray-700 hover:bg-gray-100 border border-gray-200"
                }`}
            >
          ...
          {/* Messages */}
          <div className="h-[calc(100%-12rem)] p-4 overflow-y-auto">

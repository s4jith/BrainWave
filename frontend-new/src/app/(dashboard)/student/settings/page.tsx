...
// ── Student Settings – profile info and password change ──────────────────────
export default function StudentSettingsPage() {
    ...
    const handleSaveProfile = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log('[settings] Profile updated for:', user?.email);
        } catch {
    ...
    const handleChangePassword = async (e: React.FormEvent) => {
        ...
        try {
            await apiClient.post('/api/auth/change-password', {
                user_id: user?.id, old_password: oldPwd, new_password: newPwd,
            });
            setPwdMsg({ type: 'success', text: 'Password changed successfully.' });
            setOldPwd(''); setNewPwd(''); setConfirmPwd('');
            console.log('[settings] Password changed for:', user?.email);
        } catch {
        ...
        <DashboardLayout>
            ...
            <div className="mx-auto max-w-2xl space-y-6">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input value={email} disabled
                                    className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 pl-9 py-2.5 text-sm text-gray-400 dark:border-gray-700 dark:bg-gray-700/50 cursor-not-allowed"
                                />
                            </div>
                            <p className="mt-1 text-xs text-gray-400">Email cannot be changed. Contact admin for help.</p>
                        </div>
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    {pwdMsg && (
                        <AlertBanner
                            variant={pwdMsg.type === 'success' ? 'success' : 'error'}
                            message={pwdMsg.text}
                            className="mb-4"
                        />
                    )}
                    ...
                    <form onSubmit={handleChangePassword} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input type={showOld ? 'text' : 'password'} value={oldPwd} onChange={(e) => setOldPwd(e.target.value)} required

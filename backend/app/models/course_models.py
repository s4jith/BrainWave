...
class ContentItem(BaseModel):
    ...
    url: Optional[str] = None  # Cloudinary URL for files
    content: Optional[str] = None  # For text content
    duration_minutes: int = Field(default=0, ge=0)
    order: int = Field(default=0, ge=0)
    is_required: bool = True
    assessment_id: Optional[str] = None  # Reference to assessment if type is quiz
...
class CourseResponse(BaseModel):
    ...
    average_rating: float = 0.0
    total_enrollments: int = 0
    is_enrolled: bool = False  # Set based on current user
    created_at: datetime
    updated_at: datetime

...
class PhysicsFormulaExtractor:
    ...
    def __init__(self):
        ...
        self.formula_patterns = [
            ...
            (r'p\s*=\s*mv', r'$p = mv$'),
            (r'W\s*=\s*F\s*[×·]\s*d', r'$W = F \cdot d$'),
            (r'P\s*=\s*W/t', r'$P = \frac{W}{t}$'),
            ...
            (r'Q\s*=\s*It', r'$Q = It$'),
            (r'F\s*=\s*k\s*q₁q₂/r²', r'$F = \frac{kq_1q_2}{r^2}$'),
            (r'E\s*=\s*F/q', r'$E = \frac{F}{q}$'),
            (r'C\s*=\s*Q/V', r'$C = \frac{Q}{V}$'),
            (r'F\s*=\s*BIL', r'$F = BIL$'),
            (r'F\s*=\s*qvB', r'$F = qvB$'),
            (r'Φ\s*=\s*BA', r'$\Phi = BA$'),
            (r'ε\s*=\s*-dΦ/dt', r'$\varepsilon = -\frac{d\Phi}{dt}$'),
            
            (r'1/f\s*=\s*1/v\s*[+-]\s*1/u', r'$\frac{1}{f} = \frac{1}{v} \pm \frac{1}{u}$'),
            (r'1/v\s*\+\s*1/u\s*=\s*1/f', r'$\frac{1}{v} + \frac{1}{u} = \frac{1}{f}$'),
            (r'n\s*=\s*c/v', r'$n = \frac{c}{v}$'),
            (r'n₁\s*sin\s*θ₁\s*=\s*n₂\s*sin\s*θ₂', r'$n_1 \sin\theta_1 = n_2 \sin\theta_2$'),
            (r'm\s*=\s*-v/u', r'$m = -\frac{v}{u}$'),
            (r'm\s*=\s*h₂/h₁', r'$m = \frac{h_2}{h_1}$'),
            
            (r'PV\s*=\s*nRT', r'$PV = nRT$'),
            (r'Q\s*=\s*mcΔT', r'$Q = mc\Delta T$'),
            (r'ΔU\s*=\s*Q\s*-\s*W', r'$\Delta U = Q - W$'),
            (r'η\s*=\s*W/Q', r'$\eta = \frac{W}{Q}$'),
            (r'η\s*=\s*1\s*-\s*T₂/T₁', r'$\eta = 1 - \frac{T_2}{T_1}$'),
            
            (r'v\s*=\s*fλ', r'$v = f\lambda$'),
            (r'v\s*=\s*f\s*λ', r'$v = f\lambda$'),
            (r'v\s*=\s*√\(T/μ\)', r'$v = \sqrt{\frac{T}{\mu}}$'),
            (r'I\s*∝\s*A²', r'$I \propto A^2$'),
            (r'β\s*=\s*10\s*log₁₀\(I/I₀\)', r'$\beta = 10 \log_{10}\left(\frac{I}{I_0}\right)$'),
            
            (r'E\s*=\s*mc²', r'$E = mc^2$'),
            (r'E\s*=\s*hf', r'$E = hf$'),
            (r'E\s*=\s*hν', r'$E = h\nu$'),
            (r'λ\s*=\s*h/p', r'$\lambda = \frac{h}{p}$'),
            (r'KEmax\s*=\s*hf\s*-\s*φ', r'$KE_{max} = hf - \phi$'),
            (r'1/λ\s*=\s*R\(1/n₁²\s*-\s*1/n₂²\)', r'$\frac{1}{\lambda} = R\left(\frac{1}{n_1^2} - \frac{1}{n_2^2}\right)$'),
            
            (r'N\s*=\s*N₀e^-λt', r'$N = N_0e^{-\lambda t}$'),
            (r't₁/₂\s*=\s*0\.693/λ', r'$t_{1/2} = \frac{0.693}{\lambda}$'),
            
            (r'(\w+)\s*=\s*([^,\n]{5,50})', self._to_latex_generic),
            (r'([a-zA-Z][₀-₉]*)\s*/\s*([a-zA-Z][₀-₉]*)', self._to_fraction),
            (r'([a-zA-Z])²', r'$\1^2$'),
            (r'([a-zA-Z])³', r'$\1^3$'),
            (r'√([a-zA-Z]+)', r'$\sqrt{\1}$'),
        ]
